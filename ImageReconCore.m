    ...
    obj.views = acqViews;
        ...
        function obj = buildimages(obj)
            ...
            if obj.TwoDimensional==1
               ...
               kpad = padarray(obj.complexkspace, [round(upscale_factor_phase) round(upscale_factor_read)], 0);
obj.compleximage = ifft2c(kpad);

maps = estimate_maps_from_acs_walsh(kpad, obj.multichannel_recon, obj);
obj.magimage = abs(combine_with_maps(obj.compleximage, maps, obj.multichannel_recon, obj));
               % obj.magimage = abs(combine_channels(obj.compleximage,obj.multichannel_recon,obj));
                obj.magkspace = rssq(obj.complexkspace,6);
                ...
                upscale_factor_read = double(obj.fft_size-obj.samples)/2;
                upscale_factor_phase = double((obj.fft_size*(obj.views/obj.samples))-obj.views)/2;
                % opts = struct('mode','scalar','use_outer_fraction',0.35);
                % obj.complexkspace = cancel_artifact_with_reference_coil( obj.complexkspace);
                obj.complexkspace = (windowkspace(obj.originalcomplexkspace,obj.window_size,obj.window_function)); %perform the kspace windowing first
